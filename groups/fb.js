...
const fb = 'fb';
const group_name = fb;
const group_id = config.fb.group_id; // ID группы в которой включен бот
const vk = new VK({
    token: config.fb.group_token,
    lang: "ru",
...
const COLL_NAME = config.fb.dataBase;
const donate_app = config.fb.donate_app;
const donate_app_id = config.fb.donate_app_id;
...
/*----------------------------------------------------------------------------------------------------------*/
console.log("[Фото Батл] Бот успешно загружен!"); // Сообщение в консоль
/*----------------------------------------------------------------------------------------------------------*/
...
updates.on('message', async(msg, next) => {
    ...
    if (msg.hasAttachments()) {
        if (msg.attachments[0].toString() == 'market-189152994_3844620') {
